...
from Helper_sPODG import subsample, get_T, central_FDMatrix, central_FD2Matrix
...
impath = "./data/sPODG/FRTO/Lagr/adaptive/problem=" + str(problem) + "/" + TYPE + "=" + str(VAL) + "/"  # for data
immpath = "./plots/sPODG/FRTO/Lagr/adaptive/problem=" + str(problem) + "/" + TYPE + "=" + str(VAL) + "/"  # for plots
...
kwargs = {
    ...
    'rel_control_norm_delta': 1e-6,  # Threshold for the convergence criteria based on control norm change
}
...
for opt_step in range(kwargs['opt_iter']):
    ...
    if stag or opt_step == 0:
        ...
        delta_init = calc_shift(qs, q0, wf.X, wf.t)
        _, T = get_T(delta_init, wf.X, wf.t, interp_order=kwargs['trafo_interp_order'])

        qs_org_s = T.reverse(qs)

        V, qs_s_POD = compute_red_basis(qs_org_s, **kwargs)
        Nm = V.shape[1]
        err = np.linalg.norm(qs_org_s - qs_s_POD) / np.linalg.norm(qs_org_s)
        ...
        Vd_p, Wd_p, U_dp, lhs_p, rhs_p, c_p = wf.mat_primal_sPODG_FRTO(T_delta, V, A_p, psi, D, D2,
...
np.save(impath + 'basis_refine_cntr_list.npy', basis_refine_cntr_list)
