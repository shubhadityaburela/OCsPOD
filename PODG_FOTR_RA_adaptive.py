...
impath = "./data/PODG_RA_adaptive/" + conv_crit + "/" + tar_for_bas + "/" + "problem=" + str(problem) + "/" + TYPE + "=" + str(VAL) + "/"  # for data
immpath = "./plots/PODG_RA_adaptive/" + conv_crit + "/" + tar_for_bas + "/" + "problem=" + str(problem) + "/" + TYPE + "=" + str(VAL) + "/"  # for plots
            ...
            if not kwargs['use_TWBT']:
                ...
                if len(J_opt_FOM_list) >= 5:
                    ...
                    if all(J_opt_FOM_list[-5 + i] < J_opt_FOM_list[-5 + i + 1] for i in range(4)):
                        ...
                        if BB_cntr >= 2:
                            print("\n\n-------------------------------")
                            print(
                                f"Barzilai Borwein Stagnated !!!!!! even after 2 consecutive basis updates thus exiting at itr: {opt_step} with "
                                f"J_ROM : {J}, J_FOM: {JJ}, ||dL_du||_{opt_step} / ||dL_du||_0 = {dL_du_norm / dL_du_norm_list[0]}")
                            break
                        print("\n\n-------------------------------")
                        print(
                ...
                if stag:
                    ...
                    if stag_cntr >= 2:
                        print("\n-------------------------------")
                        print(
                            f"Armijo Stagnated !!!!!! even after 2 consecutive basis updates thus exiting at itr: {opt_step} with "
                            f"J_ROM : {J}, J_FOM: {JJ}, ||dL_du||_{opt_step} / ||dL_du||_0 = {dL_du_norm / dL_du_norm_list[0]}")
                        break
                    print("\n-------------------------------")
                    print(
