...
def Update_Control_PODG_FRTO_adaptive(f, a0_primal, as_adj, qs_target, V, Ar, psir, J_prev, wf, **kwargs):
    print("Armijo iterations.........")
    count = 0
    itr = 5
    omega = kwargs['omega']
...
def Update_Control_PODG_FRTO_adaptive_TWBT(f, a0_primal, as_adj, qs_target, V, Ar, psir, J_prev, omega_prev, wf, **kwargs):
    ...
    # Checking the Armijo condition
    f_new = f - omega * dL_du
    as_ = wf.TI_primal_PODG_FOTR(a0_primal, f_new, Ar, psir)
    J = Calc_Cost_PODG(V, as_, qs_target, f_new, **kwargs)
    dJ = J_prev - kwargs['delta'] * omega * dL_du_norm_square
    if J < dJ:  # If Armijo satisfied
        n = 0
...
def Update_Control_sPODG_FRTO_adaptive(f, lhs, rhs, c, Vd_p, a0_primal, as_, as_adj, qs_target, delta_s, J_prev,
    ...
    for k in range(kwargs['Armijo_iter']):
        ...
        # Solve the primal equation
        as__, _ = wf.TI_primal_sPODG_FRTO(lhs, rhs, c, a0_primal, f_new, delta_s)
        intIds_k, weights_k = findIntervals(delta_s, as__[-1, :])

        if np.isnan(as__).any() and k < kwargs['Armijo_iter'] - 1:
            print(f"Warning!!! step size omega = {omega} too large!", f"Reducing the step size at iter={k + 1}")
            omega = omega / kwargs['omega_decr']
        elif np.isnan(as__).any() and k == kwargs['Armijo_iter'] - 1:
            print("With the given Armijo iterations the procedure did not converge. Increase the max_Armijo_iter")
            exit()
        else:
            J = Calc_Cost_sPODG(Vd_p, as__, qs_target, f_new, intIds_k, weights_k, **kwargs)
            dJ = J_prev - kwargs['delta'] * omega * dL_du_norm_square
            if J < dJ:
                J_opt = J
                f_opt = f_new
...
def Update_Control_sPODG_FRTO_adaptive_TWBT(f, lhs, rhs, c, Vd_p, a0_primal, as_, as_adj, qs_target, delta_s, J_prev, omega_prev,
    ...
    f_new = f - omega * dL_du
    as__, _ = wf.TI_primal_sPODG_FRTO(lhs, rhs, c, a0_primal, f_new, delta_s)
    intIds_a, weights_a = findIntervals(delta_s, as__[-1, :])
    J = Calc_Cost_sPODG(Vd_p, as__, qs_target, f_new, intIds_a, weights_a, **kwargs)
    dJ = J_prev - kwargs['delta'] * omega * dL_du_norm_square
    if J < dJ:  # If Armijo satisfied
        n = 0
        while True:
            ...
            f_new = f - omega * dL_du
            as__, _ = wf.TI_primal_sPODG_FRTO(lhs, rhs, c, a0_primal, f_new, delta_s)
            intIds_n, weights_n = findIntervals(delta_s, as__[-1, :])
            J = Calc_Cost_sPODG(Vd_p, as__, qs_target, f_new, intIds_n, weights_n, **kwargs)
            dJ = J_prev - kwargs['delta'] * omega * dL_du_norm_square
            if J >= dJ:
                print(
                    f"Armijo satisfied and the inner loop converged at the {n}th step with final omega = {omega_final}")
        ...
        while True:
            ...
            if J < dJ:  # If Armijo satisfied
                if omega < kwargs['omega_cutoff']:
                    print(f"Omega went below the omega cutoff on the {k}th step, thus exiting the Armijo loop !!!!!!")
                    return f_new, J, dL_du_norm, omega, True
                else:
                    print(f"Armijo converged on the {k}th step with final omega = {omega}")
                    return f_new, J, dL_du_norm, omega, False
