...
from Update import (
    get_BB_step, Update_Control_BB_kdvb, Update_Control_sPODG_FOTR_RA_TWBT_kdvb,
)
...
def build_dirs(prefix, common_basis, reg_tuple, CTC_mask, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data/sPODG_FOTR_kdvb", cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots/sPODG_FOTR_kdvb", cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"{TYPE}={VAL}")
...
if __name__ == "__main__":
    ...
    lhs_p, rhs_p, deim_p, c_p = mat_primal_sPODG_FOTR_kdvb(T_delta, Vd_p, Wd_p, Ud_p, V_deim_p,
                                                           ...
                                                           delta_s)
    lhs_a, rhs_a, deim_a, deim_a_hl, t_a = mat_adjoint_sPODG_FOTR_kdvb(T_delta, Vd_a, Wd_a, Ud_a, Vd_p, Wd_p, V_deim_a,
 ...
 # v = np.max(np.abs(qs_full_deim_a_s))
 #    fig, axes = plt.subplots(1, 1, figsize=(4, 4))
 #    pcm0 = axes.pcolormesh(qs_full_deim_a_s.T, vmin=-v, vmax=v, cmap='RdBu_r', shading='auto')
 #    fig.colorbar(pcm0, ax=axes)
 #    plt.show()
 #    exit()