...
from Update import (
    get_BB_step, Update_Control_sPODG_FRTO_TWBT_kdvb, Update_Control_BB_kdvb,
)
...
def build_dirs(prefix, common_basis, reg_tuple, CTC_mask, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data/sPODG_FRTO_kdvb_adaptive", cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots/sPODG_FRTO_kdvb_adaptive", cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"{TYPE}={VAL}")
    ...
    try:
        for opt_step in range(kwargs['opt_iter']):
            ...
            if stag or opt_step % 10 == 0:
                ...
                lhs_p, rhs_p, deim_p, deim_p_hl, c_p, tar_a = mat_primal_sPODG_FRTO_kdvb(T_delta, Vd_p, Wd_p, Ud_p,
                                                                                         V_deim,
                                                                                         kwargs['shift_sample'],
