...
from PODG_solver import (IC_primal_kdvb_PODG_FRTO,
                         IC_adjoint_kdvb_PODG_FRTO, mat_primal_kdvb_PODG_FRTO, mat_adjoint_kdvb_PODG_FRTO,
                         TI_primal_kdvb_PODG_FRTO_impl,
                         TI_adjoint_kdvb_PODG_FRTO_impl, TI_primal_kdvb_PODG_FRTO_expl
                         )
...
def build_dirs(prefix, common_basis, reg_tuple, CTC_mask, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data/PODG_FRTO_kdvb_adaptive", cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots/PODG_FRTO_kdvb_adaptive", cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"{TYPE}={VAL}")
        ...
        for opt_step in range(kwargs['opt_iter']):
            ...
            if stag or opt_step == 0:
                ...
                J_l_ROM_adjoint = np.eye(Nm) + 0.5 * kdvb.dt * (
                        - primal_mat.A_r.T + params_primal['gamma'] * primal_mat.D_3r.T - params_primal[
                    'nu'] * primal_mat.D_2r.T)
