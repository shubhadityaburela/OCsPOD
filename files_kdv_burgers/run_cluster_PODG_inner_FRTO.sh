...
if [ "$script_type" = "adaptive" ]; then
    if [ "$param_type" = "tol" ]; then
        python3 PODG_FRTO_kdvb_adaptive.py $common_basis 20000 "/work/burela" 0 1e-3 False --tol $tol1 $tol2
    else
        python3 PODG_FRTO_kdvb_adaptive.py $common_basis 20000 "/work/burela" 0 1e-3 False --modes $mode1 $mode2
    fi
else
    if [ "$param_type" = "tol" ]; then
        python3 PODG_FRTO_kdvb.py $common_basis 20000 "/work/burela" 0 1e-3 False --tol $tol1 $tol2
    else
        python3 PODG_FRTO_kdvb.py $common_basis 20000 "/work/burela" 0 1e-3 False --modes $mode1 $mode2
    fi
fi
