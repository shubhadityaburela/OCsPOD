...
def Calc_Cost(q, q_target, f, **kwargs):
    q_res = q - q_target
    cost = 1 / 2 * (integrate_cost(q_res, **kwargs)) + (kwargs['lamda'] / 2) * (trapezoidal_integration(f, **kwargs))
...
def Calc_Cost_PODG(V, as_, qs_target, f, **kwargs):
    ...
    cost = 1 / 2 * (integrate_cost(q_res, **kwargs)) + (kwargs['lamda'] / 2) * (
...
def Calc_Cost_sPODG_newcost(as_, as_target, z_target, f, **kwargs):
    ...
    cost = 1 / 2 * (integrate_cost_TA(a_res, **kwargs)) + 1 / 2 * (integrate_cost_TA(z_res, **kwargs)) \
           + (kwargs['lamda'] / 2) * (trapezoidal_integration_control(f, **kwargs))
