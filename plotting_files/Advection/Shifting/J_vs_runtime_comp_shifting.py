...
def extract(problem: str,
    ...
    base_path = os.path.join(
        "/Users/shubhadityaburela/Python/Paper(4.1 + 4.2)_OCsPOD/OCsPOD/results_advection_FOTR/data/", problem,
...
case3 = "PODG_FOTR_adaptive"
case4 = "sPODG_FOTR_adaptive"
...
t_min_1 = min([min(t[:, 0]) for t in POD_modes_data_3])
t_max_1 = max([max(t[:, 0]) for t in POD_modes_data_3])
t_min_2 = min([min(t[:, 0]) for t in sPOD_modes_data_3])
t_max_2 = max([max(t[:, 0]) for t in sPOD_modes_data_3])
t_min_3 = min(FOM_t[:, 0])
t_max_3 = max(FOM_t[:, 0])
t_min = min(t_min_1, t_min_2, t_min_3)
t_max = max(t_max_1, t_max_2, t_max_3)
common_t_grid = np.linspace(t_min, t_max, 500)
