...
if [ "$param_type" = "tol" ]; then
    python3 files_advection/sPODG_FOTR_FA.py $type_of_problem $problem_number $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 $CTC_mask --tol $tol_value
else
    python3 files_advection/sPODG_FOTR_FA.py $type_of_problem $problem_number $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 $CTC_mask --modes $mode
fi
