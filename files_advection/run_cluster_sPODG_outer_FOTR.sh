  ...
  for common_basis in True False; do
    ...
    if [ "$type_of_problem" = "Constant_shift" ]; then
      for ms in "${mode_sets[@]}"; do
        ...
        sbatch run_cluster_sPODG_inner_FOTR.sh "$type_of_problem" "$common_basis" "modes" "$CTC_mask" "$mode1" "$mode2" "$script_type" "$grid_str"
      done
    else
      for tol in "${tolerances[@]}"; do
        ...
        sbatch run_cluster_sPODG_inner_FOTR.sh "$type_of_problem" "$common_basis" "tol" "$CTC_mask" "$tol" "$script_type" "$grid_str"
      done
    fi
