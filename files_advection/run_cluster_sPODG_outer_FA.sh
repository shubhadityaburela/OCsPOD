...
for common_basis in True False; do
  ...
  if [ "$type_of_problem" = "Constant_shift" ]; then
    for ms in "${mode_sets[@]}"; do
      ...
      sbatch run_cluster_sPODG_inner_FA.sh "$type_of_problem" "$common_basis" "modes" "$CTC_mask" "$mode1" "$problem_number" "$grid_str"
    done
  else
    for ms in "${mode_sets[@]}"; do
      read -r mode1 <<< "$ms"
...
#      sbatch run_cluster_sPODG_inner_FA.sh "$type_of_problem" "$common_basis" "tol" "$CTC_mask" "$tol" "$problem_number" "$grid_str"
#    done
  fi
