...
def build_dirs(prefix, type_of_problem, reg_tuple, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data", type_of_problem, "PODG_Armijo", reg_str,
                            f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots", type_of_problem, "PODG_Armijo", reg_str,
                            f"{TYPE}={VAL}")
...
if __name__ == "__main__":
    ...
    try:
        for opt_step in range(kwargs['opt_iter']):
            ...
            V, qsPOD = compute_red_basis(qs_opt_full, equation="primal", **kwargs)
            Nm = V.shape[1]
            err = np.linalg.norm(qs_opt_full - qsPOD) / np.linalg.norm(qs_opt_full)
