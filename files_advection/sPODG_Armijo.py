...
from Grads import Calc_Grad_PODG_smooth, Calc_Grad_mapping, Calc_Grad_smooth
from Helper import ControlSelectionMatrix, compute_red_basis, L2norm_ROM, check_weak_divergence, L2inner_prod, \
    calc_shift
from Helper_sPODG import central_FDMatrix, subsample, central_FD2Matrix, get_T, make_V_W_U_delta
...
from Update import Update_Control_PODG_FRTO_TWBT, get_BB_step, Update_Control_BB, Update_Control_sPODG_FRTO_TWBT
from grid_params import advection, advection_3
from Plots import PlotFlow
from sPODG_solver import IC_primal_sPODG_FRTO, mat_primal_sPODG_FRTO
...
def build_dirs(prefix, type_of_problem, reg_tuple, TYPE, VAL):
    reg_str = f"L1={reg_tuple[0]}_L2={reg_tuple[1]}"
    data_dir = os.path.join(prefix, "data", type_of_problem, "sPODG_Armijo", reg_str,
                            f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots", type_of_problem, "sPODG_Armijo", reg_str,
                            f"{TYPE}={VAL}")
...
if __name__ == "__main__":
    ...
    print("Type of basis computation: Armijo study")
    ...
    try:
        for opt_step in range(kwargs['opt_iter']):
            ...
            # Compute the basis
            snap_cat_p_s = np.column_stack([np.roll(qs_opt_full[:, j], -j, axis=0) for j in range(qs_opt_full.shape[1])])
            ...
            # ───── Compute costs ─────
            J_s, J_ns = Calc_Cost(qs_opt_full, qs_target, f, C, kwargs['dx'], kwargs['dt'],
            ...
            qs_adj_full = TI_adjoint(q0_adj, qs_opt_full, qs_target, None, A_a, None, C, wf.Nx, wf.dx, wf.Nt, wf.dt,
                                     scheme="Explicit_Euler")
            t_4 = perf_counter()

            # ───── Compute the smooth gradient + the generalized gradient mapping ─────
            dL_du_s = Calc_Grad_smooth(psi, f, qs_adj_full, kwargs['lamda_l2'])
