...
from PODG_solver import (IC_primal_PODG_FOTR,
                         IC_adjoint_PODG_FOTR, mat_primal_PODG_FOTR, mat_adjoint_PODG_FOTR, TI_primal_PODG_FOTR,
                         TI_adjoint_PODG_FOTR
                         )
from Update import get_BB_step, Update_Control_BB, Update_Control_PODG_FOTR_RA_TWBT
...
def build_dirs(prefix, type_of_problem, common_basis, reg_tuple, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data", type_of_problem, "PODG_FOTR", cb_str, reg_str,
                            f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots", type_of_problem, "PODG_FOTR", cb_str, reg_str,
                            f"{TYPE}={VAL}")
...
if __name__ == "__main__":
    ...
    try:
        for opt_step in range(kwargs['opt_iter']):
            ...
            as_p = TI_primal_PODG_FOTR(a_p, f, Ar_p, psir_p, kwargs['Nt'], kwargs['dt'])
            t_1 = perf_counter()
            ...
            as_adj = TI_adjoint_PODG_FOTR(a_a, as_p, M_f, A_f, LU_M_f, V_aTV_p, Tarr, kwargs['Nt'], kwargs['dt'],
            ...
            if kwargs['offline_online_err_check']:
                ...
                print(
                    "The discrepancy between the offline error and the online error starts growing if there are a "
                ...
                qs_adj_opt_full = TI_adjoint(q0_adj, qs_full, qs_target, None, A_a, None, C, wf.Nx, wf.dx, wf.Nt, wf.dt,
                                             scheme="Explicit_Euler")
