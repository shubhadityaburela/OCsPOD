...
if [ "$script_type" = "adaptive" ]; then
    if [ "$param_type" = "tol" ]; then
        python3 files_advection/PODG_FOTR_adaptive.py $type_of_problem $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 --tol $tol_value
    else
        python3 files_advection/PODG_FOTR_adaptive.py $type_of_problem $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 --modes $mode1 $mode2
    fi
else
    if [ "$param_type" = "tol" ]; then
        python3 files_advection/PODG_FOTR.py $type_of_problem $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 --tol $tol_value
    else
        python3 files_advection/PODG_FOTR.py $type_of_problem $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 --modes $mode1 $mode2
    fi
fi
