#!/bin/bash
#SBATCH --job-name=sPODG_Armijo       # Job name
...
if [ "$param_type" = "tol" ]; then
    tol_value=$3      # Single tolerance value
else
...
if [ "$param_type" = "tol" ]; then
    python3 files_advection/sPODG_Armijo.py $type_of_problem "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 --tol $tol_value
else
    python3 files_advection/sPODG_Armijo.py $type_of_problem "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 --modes $mode
fi
