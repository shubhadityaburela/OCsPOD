...
from PODG_solver import IC_adjoint_PODG_FOTR, mat_adjoint_PODG_FOTR_mix, TI_adjoint_PODG_FOTR_mix
...
def build_dirs(prefix, common_basis, reg_tuple, CTC_mask, interp_scheme, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data/sPODG_PODG_adaptive", cb_str, reg_str, f"CTC_mask={CTC_mask}", interp_str,
                            f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots/sPODG_PODG_adaptive", cb_str, reg_str, f"CTC_mask={CTC_mask}", interp_str,
                            f"{TYPE}={VAL}")
