...
mode_sets=(
  "2 2"   "5 5"  "8 8"  "10 10"  "12 12"  "15 15"  "20 20"  "25 25"
...
run_single_job() {
    ...
    if [[ "$script_type" == "adaptive" ]]; then
        py_script="files_advection/sPODG_FOTR_adaptive.py"
    else
        py_script="files_advection/sPODG_FOTR.py"
    fi

    ts=$(date +"%Y%m%dT%H%M%S")
    if [[ "$param_type" == "tol" ]]; then
        local tol_value="$4"
        logfile="$PROJECT_ROOT/data/logs/sPODG_${type_of_problem}_${script_type}_common${common_basis}_tol${tol_value}_g${g1}-${g2}-${g3}_${ts}.log"
        ...
        local mode1="$4"
        local mode2="$5"
        logfile="$PROJECT_ROOT/data/logs/sPODG_${type_of_problem}_${script_type}_common${common_basis}_modes${mode1}-${mode2}_g${g1}-${g2}-${g3}_${ts}.log"
...
echo "Starting local sPODG batch (project root: $PROJECT_ROOT), type_of_problem='$type_of_problem'"
...
echo "Local sPODG batch complete."
