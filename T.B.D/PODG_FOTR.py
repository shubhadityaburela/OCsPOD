...
from Update import Update_Control_PODG_FOTR
...
impath = "./data/PODG/FOTR/Nm=140/"
immpath = "./plots/PODG/FOTR/Nm=140/"
os.makedirs(impath, exist_ok=True)
n_rom = 140  # Number of modes
...
for opt_step in range(kwargs['opt_iter']):
    ...
    V_p, qs_POD = compute_red_basis(qs, n_rom)
    ...
    time_odeint = perf_counter()  # save timing
    '''
    Backward calculation with adjoint for basis update
    '''
    qs_adj = wf.TI_adjoint(q0_adj, f, qs, qs_target, A_a)

    # Compute the reduced basis
    V_a, qs_adj_POD = compute_red_basis(qs_adj, n_rom)
    err = np.linalg.norm(qs_adj - qs_adj_POD) / np.linalg.norm(qs_adj)
    print(f"Relative error for adjoint: {err}, with n_rom_adjoint: {n_rom}")

    # Initial condition for dynamical simulation
    a_a = wf.IC_adjoint_PODG_FOTR(V_a, q0_adj)

    # Construct the adjoint system matrices for the POD-Galerkin approach
    Ar_a, Tr_a, Tarr_a, psir_a = wf.mat_adjoint_PODG_FOTR(V_a, A_p, V_p, qs_target, psi)
    ...
    '''
    time_odeint = perf_counter()  # save timing
    as_adj = wf.TI_adjoint_PODG_FOTR(a_a, f, as_, Ar_a, Tr_a, Tarr_a)
    time_odeint = perf_counter() - time_odeint
    ...
    f, J_opt, dL_du, stag = Update_Control_PODG_FOTR(f, a_p, as_adj, qs_target, V_p, Ar_p, psir_p, psir_a, J,
                                                     wf=wf, **kwargs)
