"""
This file is the version with ROM adjoint (Barzilai Borwein). It can handle both the scenarios.
1. Fixed tolerance
2. Fixed modes
"""
...
from Update import Update_Control_BB, Update_Control_sPODG_FOTR_RA_TWBT, Update_Control_sPODG_FOTR_RA_BB
from advection import advection
from Plots import PlotFlow
from Helper import ControlSelectionMatrix_advection, compute_red_basis, calc_shift, is_contiguous
from Helper_sPODG import subsample, findIntervals, get_T, central_FDMatrix, get_approx_state_sPODG
...
impath = "./data/sPODG_RA/problem=" + str(problem) + "/" + TYPE + "=" + str(VAL) + "/"  # for data
immpath = "./plots/sPODG_RA/problem=" + str(problem) + "/" + TYPE + "=" + str(VAL) + "/"  # for plots
...
kwargs = {
    ...
    'epsilon': 5e-2,  # Error measure for basis refinement
}
...
for opt_step in range(kwargs['opt_iter']):
    ...
    epsilon = np.linalg.norm(qq - qs) / np.linalg.norm(qs)
    print(f"The error measure for basis refinement: {epsilon}")
    ...
    '''
    as_, intIds, weights = wf.TI_primal_sPODG_FOTR(lhs_p, rhs_p, c_p, a_p, f, delta_s, modes=Nm_p)
    ...
    if opt_step == 0 or epsilon > kwargs['epsilon']:
        ...
        V_a, qs_adj_s_POD = compute_red_basis(qs_adj_s, **kwargs)
        Nm_a = V_a.shape[1]
        ...
        a_a = wf.IC_adjoint_sPODG_FOTR(Nm_a, as_[-1, -1])
    ...
    '''
    if opt_step == 0:
        fNew, J_opt, dL_du_Old, dL_du_norm, omega, qq, stag = Update_Control_sPODG_FOTR_RA_TWBT(f, lhs_p, rhs_p, c_p,
                                                                                                a_p,
                                                                                                Vd_a,
                                                                                                as_adj,
                                                                                                qs_target, delta_s,
        ...
        fNew, dL_du_Old, dL_du_norm = Update_Control_sPODG_FOTR_RA_BB(fOld, fNew, dL_du_Old, Vd_a, as_adj, psi,
                                                                      opt_step, intIds, weights, **kwargs)
        as__, Ids, Weigs = wf.TI_primal_sPODG_FOTR(lhs_p, rhs_p, c_p, a_p, fNew, delta_s, Nm_p)
        qq = get_approx_state_sPODG(Vd_p, fNew, as_[:-1], Ids, Weigs, kwargs['Nx'], kwargs['Nt'])
