"""
This file is the version with FOM adjoint (adaptive basis update for primal). It can handle both the scenarios.
1. Fixed tolerance
2. Fixed modes
"""
...
from Update import Update_Control_sPODG_FOTR_FA_TWBT, Update_Control_sPODG_FOTR_FA_TWBT_dot
from advection import advection
from Plots import PlotFlow
from Helper import ControlSelectionMatrix_advection, compute_red_basis, calc_shift, is_contiguous
from Helper_sPODG import subsample, findIntervals, get_T, central_FDMatrix, compute_residual
...
parser.add_argument("epsilon", type=float, help="Specify the basis refinement tolerance threshold")
...
print(f"Given basis refinement epsilon value: {args.epsilon}")
...
kwargs = {
    ...
    'epsilon': epsilon,  # Error measure for basis refinement
}
#%%
...
for opt_step in range(kwargs['opt_iter']):
    ...
    '''
     Update Control
    '''
    f, J_opt, _, dL_du_norm, omega, qq, as_res, intIds_res, weights_res, as_dot_res, stag = \
    ...
    residual = compute_residual(D, A_p, Vd_p, psi, as_res[:-1], as_dot_res, f, intIds_res,
