...
def IC_primal_target_extinguish(X, Lxi, offset, variance):
    q = 0.25 * np.exp(-((X - Lxi / offset) ** 2) / variance)
    return q
...
def build_dirs(prefix, reg_tuple, CTC_mask, problem):
    ...
    data_dir = os.path.join(prefix, "data/FOM_extinguish", reg_str, f"CTC_mask={CTC_mask}", f"problem={problem}")
    plot_dir = os.path.join(prefix, "plots/FOM_extinguish", reg_str, f"CTC_mask={CTC_mask}", f"problem={problem}")
