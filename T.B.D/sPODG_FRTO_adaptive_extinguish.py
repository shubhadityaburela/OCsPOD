...
from grid_params_test import advection
...
@njit
def IC_primal_extinguish(X, Lxi, offset, variance):
    q = 1 * np.exp(-((X - Lxi / offset) ** 2) / variance)
    return q


@njit
def IC_primal_target_extinguish(X, Lxi, offset, variance):
    q = 0.1 * np.exp(-((X - Lxi / offset) ** 2) / variance)
    return q
...
def setup_advection(problem):
    Nxi, Nt = 2000 // 1, 4000 // 1
    if problem == 1:
        wf = advection(Nxi=Nxi, timesteps=Nt,
                       cfl=8 / 6, tilt_from=3 * Nt // 4,
                       v_x=0.6, v_x_t=1.0,
                       variance=20, offset=12)
    elif problem == 2:
        wf = advection(Nxi=Nxi, timesteps=Nt,
                       cfl=0.4, tilt_from=3 * Nt // 4,
                       v_x=220.0, v_x_t=1.0,
...
def build_dirs(prefix, common_basis, reg_tuple, CTC_mask, interp_scheme, problem, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data/sPODG_FRTO_adaptive_extinguish", cb_str, reg_str, f"CTC_mask={CTC_mask}", interp_str,
                            f"problem={problem}", f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots/sPODG_FRTO_adaptive_extinguish", cb_str, reg_str, f"CTC_mask={CTC_mask}", interp_str,
                            f"problem={problem}", f"{TYPE}={VAL}")
...
if __name__ == "__main__":
    ...
    qs0 = IC_primal_extinguish(wf.X, wf.Lxi, wf.offset, wf.variance)
    qs_org = TI_primal(qs0, f, A_p, psi, wf.Nxi, wf.Nt, wf.dt)
    qs_target = np.load("qs_target.npy")
    qs_opt = np.load("qs_opt.npy")
    # a = np.exp(0.01 * wf.t)  # e.g. linear or exponential in time
    # sigma_array = 5 * (wf.t / wf.t[-1])  # ramp from 0 to sigma_final
    # for i in range(wf.Nt):
    #     qs_target[:, i] = gaussian_filter1d(a[i]*qs_target[:, i], sigma_array[i]/wf.dx)
    ...
    # frames = range(0, len(wf.t), skip)
    # fig, ax = plt.subplots(figsize=(8, 4))
    # # plot both on the same axes, with labels
    # line1, = ax.plot(wf.X, qs_opt[:, 0], lw=2, label='Controlled')
    # line2, = ax.plot(wf.X, qs_target[:, 0], lw=2, label='Target')
    # ax.set_xlim(wf.X.min(), wf.X.max())
    # ax.set_ylim(-0.1, 1)
    # ax.set_xlabel('x')
    # ax.set_ylabel('u(x,t)')
    # ax.legend()
    ...
    #     line1.set_ydata(qs_opt[:, idx])
    #     line2.set_ydata(qs_target[:, idx])
    #     title.set_text(f"t = {wf.t[idx]:.2f}")
    ...
    kwargs = {
        ...
        'lamda_l1': L1_reg,
        'lamda_l2': L2_reg,
        'delta_conv': 1e-6,  # Convergence criteria
        'delta': 1.01,  # Armijo constant
        'opt_iter': args.N_iter,  # Total iterations
        'shift_sample': 1000,  # Number of samples for shift interpolation
    ...
    try:
        for opt_step in range(kwargs['opt_iter']):
            ...
            if stag or opt_step % 50 == 0:
                ...
                qs_adj_full = TI_adjoint(q0_adj, qs_full, qs_target, None, A_a, None, C, wf.Nxi, wf.dx, wf.Nt, wf.dt,
                                         scheme="RK4")
                ...
                if kwargs['interp_scheme'] == "Lagr":
                    ...
                    if kwargs['common_basis']:
                        ...
                        qs_adj_norm_s = shift_matrix_precomputed_coeffs_multiple(qs_adj_norm, z[0], b_a, c_a, d_a,
                                                                                 kwargs['Nx'],
                                                                                 kwargs['dx'])
                        ...
                        qs_s = shift_matrix_precomputed_coeffs_multiple(qs_full, z[0], b_p, c_p, d_p, kwargs['Nx'],
                                                                        kwargs['dx'])
                ...
                if kwargs['interp_scheme'] == "Lagr":
                    ...
                    Vd_p, Wd_p, Ud_p = make_V_W_delta_CubSpl(V, delta_s, A1, D1, D2, R, kwargs['shift_sample'],
                                                             kwargs['Nx'],
                                                             kwargs['dx'], Nm)
            ...
            if ratio < 5e-3:
                ...
                if omega_bb < 0:
                    ...
                    fNew = Update_Control_sPODG_FRTO_BB(f, dL_du_s, omega_bb, kwargs['lamda_l1'])
                    stag = False
                    omega = omega_bb
            else:
                print("TWBT acting…")
    ...
    np.save("qs_opt.npy", qs_opt_full__)
