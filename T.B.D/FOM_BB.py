...
from Update import Update_Control_BB, Update_Control_TWBT
...
for opt_step in range(kwargs['opt_iter']):
    ...
    if opt_step == 0:
        ...
        fNew, J_opt, dL_du_Old, dL_du_norm, omega, stag = Update_Control_TWBT(f, q0, qs_adj, qs_target, psi, A_p, J,
                                                                              omega, wf=wf, **kwargs)
    else:
    ...
    print(
        f"J : {J}, ||dL_du|| = {dL_du_norm}, ||dL_du||_{opt_step} / ||dL_du||_0 = {dL_du_norm / dL_du_norm_list[0]}"
    )
    ...
    if opt_step == kwargs['opt_iter'] - 1:
        ...
        print(
            ...
            f"J : {J}, ||dL_du||_{opt_step} / ||dL_du||_0 = {dL_du_norm / dL_du_norm_list[0]}"
        )
        break
...
qs_opt = wf.TI_primal(q0, fNew, A_p, psi)
f_opt = psi @ fNew

# Compute the cost with the optimal control
J = Calc_Cost(qs_opt, qs_target, fNew, kwargs['dx'], kwargs['dt'], kwargs['lamda'])
J_opt_list.append(J)  # Last cost functional
print("\n")
print(f"J with respect to the newest control: {J}")
...
np.save(impath + 'f_opt_low.npy', fNew)
...
pf.plot1D_FOM_converg(J_opt_list, dL_du_norm_ratio_list, immpath=immpath)
