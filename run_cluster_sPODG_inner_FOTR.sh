...
if [ "$script_type" = "adaptive" ]; then
    if [ "$param_type" = "tol" ]; then
        python3 sPODG_FOTR_RA_adaptive.py $problem $conv_accel $target_for_basis $interp_scheme --tol $tol_value
    else
        python3 sPODG_FOTR_RA_adaptive.py $problem $conv_accel $target_for_basis $interp_scheme --modes $mode1 $mode2
    fi
else
    if [ "$param_type" = "tol" ]; then
        python3 sPODG_FOTR_RA.py $problem $conv_accel $target_for_basis $interp_scheme --tol $tol_value
    else
        python3 sPODG_FOTR_RA.py $problem $conv_accel $target_for_basis $interp_scheme --modes $mode1 $mode2
    fi
fi
