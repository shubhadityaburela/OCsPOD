...
if [ "$script_type" = "adaptive" ]; then
    if [ "$param_type" = "tol" ]; then
        python3 files_kdv/PODG_FRTO_kdv_adaptive.py $fully_nonlinear $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 $CTC_mask --tol $tol_value
    else
        python3 files_kdv/PODG_FRTO_kdv_adaptive.py $fully_nonlinear $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 $CTC_mask --modes $mode
    fi
else
    if [ "$param_type" = "tol" ]; then
        python3 files_kdv/PODG_FRTO_kdv.py $fully_nonlinear $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 $CTC_mask --tol $tol_value
    else
        python3 files_kdv/PODG_FRTO_kdv.py $fully_nonlinear $common_basis "${grid_params[@]:0:3}" 20000 "/work/burela" 0 1e-3 $CTC_mask --modes $mode
    fi
fi
