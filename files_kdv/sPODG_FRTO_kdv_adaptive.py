...
from FOM_solver import (IC_primal_kdv,
    TI_primal_kdv_impl, IC_adjoint_kdv, TI_adjoint_kdv_impl,
)
from Grads import (
...
from Update import (
    get_BB_step, Update_Control_sPODG_FRTO_TWBT_kdv,
...
from Helper import (
    ...
    calc_shift,
    L2norm_ROM,
    check_weak_divergence, ControlSelectionMatrix_kdvb, compute_deim_basis, L2inner_prod,
)
from Helper_sPODG import (
    subsample,
    get_T,
    make_V_W_U_delta, get_approx_state_sPODG,
)

from sPODG_solver import (
    IC_primal_sPODG_FRTO_kdv, IC_adjoint_sPODG_FRTO_kdv, mat_primal_sPODG_FRTO_kdv, TI_primal_sPODG_FRTO_kdv_expl,
    TI_adjoint_sPODG_FRTO_kdv_impl,
)
...
def build_dirs(prefix, fully_nonlinear, common_basis, reg_tuple, CTC_mask, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data/sPODG_FRTO_kdv_adaptive", fnl_str, cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots/sPODG_FRTO_kdv_adaptive", fnl_str, cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"{TYPE}={VAL}")
...
if __name__ == "__main__":
    ...
    # Compute shifts of the primal uncontrolled profile  (Would be always the same)
    z = calc_shift(qs_org, q0, kdv.X, kdv.t)
    ...
    running_time = []
    trunc_modes_list = []
    trunc_deim_modes_list = []
    basis_update_idx_list = []
    best_control = np.zeros_like(f)
    best_details = {'J': np.inf, 'N_iter': None, 'Nm': None, 'Nm_deim': None}
    f_last_valid = None
    ...
    try:
        for opt_step in range(kwargs['opt_iter']):
            ...
            if stag or opt_step % 20 == 0:
                basis_update_idx_list.append(opt_step)

                # ───── Forward FOM: compute FOM state qs ─────
                # Compute FOM trajectories
                qs_full = TI_primal_kdv_impl(q0, f, J_l, kdv.Nx, kdv.Nt, kdv.dt, **params_primal)
                qs_adj_full = TI_adjoint_kdv_impl(q0_adj, qs_full, qs_target, J_l_adjoint, kdv.Nx, kdv.Nt, kdv.dx,
                                                  kdv.dt, **params_adjoint)
                ...
                a_p = IC_primal_sPODG_FRTO_kdv(q0, V)
                a_a = IC_adjoint_sPODG_FRTO_kdv(Nm)
                trunc_modes_list.append(Nm)
                trunc_deim_modes_list.append(Nm_deim)
                ...
                lhs_p, rhs_p, deim_p, deim_p_hl, c_p, tar_a = mat_primal_sPODG_FRTO_kdv(T_delta, Vd_p, Wd_p, Ud_p,
                                                                                        V_deim,
                                                                                        kwargs['shift_sample'],
                                                                                        Nm, Nm_deim, params_primal,
                                                                                        params_adjoint,
                                                                                        delta_s)

            # ───── Forward ROM: compute ROM state a_p → as_p ─────
            as_p, as_dot, intIds, weights = TI_primal_sPODG_FRTO_kdv_expl(lhs_p, rhs_p, deim_p, c_p, a_p, f,
            ...
            as_adj = TI_adjoint_sPODG_FRTO_kdv_impl(a_a, f, as_p, qs_target, as_dot, lhs_p, rhs_p, deim_p, deim_p_hl,
                                                    c_p, tar_a, Vd_p, Wd_p,
            ...
            if ratio < 5e-3:
                ...
                if omega_bb < 0:
                    ...
                    fNew, omega_twbt, stag = Update_Control_sPODG_FRTO_TWBT_kdv(f, lhs_p, rhs_p, deim_p, deim_p_hl,
                                                                                c_p, Vd_p,
            ...
            if opt_step % 100 == 0:
                ...
                if diverging:
                    ...
                    f_last_valid = f.copy()
                    qs_cand = TI_primal_kdv_impl(q0, f_last_valid, J_l, kdv.Nx, kdv.Nt, kdv.dt,
                                                 **params_primal)
        ...
        to_save = {
            ...
            "trunc_deim_modes_list_at_crash": trunc_deim_modes_list,
