...
def build_dirs(prefix, common_basis, reg_tuple, CTC_mask, problem, TYPE, VAL):
    ...
    data_dir = os.path.join(prefix, "data/PODG_FOTR_RA", cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"problem={problem}", f"{TYPE}={VAL}")
    plot_dir = os.path.join(prefix, "plots/PODG_FOTR_RA", cb_str, reg_str, f"CTC_mask={CTC_mask}",
                            f"problem={problem}", f"{TYPE}={VAL}")
