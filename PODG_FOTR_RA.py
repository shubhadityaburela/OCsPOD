...
parser.add_argument("--reg",
                    ...
                    nargs=2,
                    metavar=("L1", "L2"),
                    help="Enter both L1 and L2 regularization weights (e.g. --reg 0.01 0.001)"
                    )
...
print(f"The L1 and L2 regularization parameters respectively: {args.reg}")
...
kwargs = {
    ...
    'Nt': wf.Nt,
    'n_c': n_c,
    'lamda_l1': L1_reg,  # Regularization parameter for L1 penalty
    'lamda_l2': L2_reg,  # Regularization parameter for L2 penalty
...
impath = args.dir_prefix + "/data/PODG_FOTR_RA/" + common_basis + "/" + "problem=" + str(
    problem) + "/" + TYPE + "=" + str(
    VAL) + "/"  # for data
immpath = args.dir_prefix + "/plots/PODG_FOTR_RA/" + common_basis + "/" + "problem=" + str(
    problem) + "/" + TYPE + "=" + str(
    VAL) + "/"  # for plots
...
for opt_step in range(kwargs['opt_iter']):
    ...
    '''
    J = Calc_Cost_PODG(V_p, as_, qs_target, f, kwargs['dx'], kwargs['dt'], kwargs['lamda_l1'], kwargs['lamda_l2'])
    ...
    JJ = Calc_Cost(qs_opt_full, qs_target, f, kwargs['dx'], kwargs['dt'], kwargs['lamda_l1'], kwargs['lamda_l2'])
